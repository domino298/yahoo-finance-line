...
<body>
  ...
  <script>
    ...
    function renderRows() {
      ...
      for (const item of items) {
        ...
        for (const [index, value] of cells.entries()) {
          ...
          if (index === 3) td.title = `株価時点: ${formatDateTime(item.quote_time) || "不明"}`;
    ...
    function normalizeQuote(quote) {
      if (quote.price === null || quote.price === "" || typeof quote.price === "boolean" || typeof quote.previous_close === "boolean") throw new Error("現在値がありません");
      ...
      const changePercent = calculatedPercent;
      return {
        ...quote,
        price,
    ...
    async function refreshLiveQuotes(symbolsOverride) {
      ...
      for (let index = 0; index < symbols.length; index += batchSize) {
        ...
        try {
          ...
          if (!data || !data.quotes) throw new Error("中継の応答が不正です");
          transportFailures = 0;
        } catch (error) {
          transportFailures += 1;
          data = { quotes: {} };
      ...
      for (const portfolio of payload.portfolios || []) {
        for (const item of portfolio.symbols || []) {
          ...
          item.warning = quote.warning || "";
          item.alert_direction = alertDirection(item);
          item.error = "";
        }
      }
      payload.generated_at = new Date().toISOString();
      payload.quote_time = newestJapanQuoteTime || newestQuoteTime || payload.quote_time || "";
    ...
    async function loadInitialData() {
      ...
      try {
        ...
        els.statusText.textContent = `株価時点（日本株）: ${formatDateTime(payload.quote_time) || "不明"}`;
        render();
        els.refreshButton.disabled = false;
        await refreshData();
      } catch (error) {
